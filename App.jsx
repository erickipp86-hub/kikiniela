...
export default function App() {
  ...
  const fetchAllDataSilent = async () => {
    try {
      const resGames = await fetch(`${SCRIPT_URL}?sheet=${encodeURIComponent(SHEET_NAME)}`);
      const dataGames = await resGames.json();
      if (Array.isArray(dataGames) && dataGames.length > 0) {
        const formattedGames = dataGames.map((item, index) => ({
          id: Number(item.ID) || index + 1,
          week: String(item.Semana || '3').trim(),
          home: item.Local,
          away: item.Visitante,
  ...
  const availableWeeks = games.length > 0 ? Array.from(new Set(games.map(g => g.week))).sort((a, b) => Number(a) - Number(b)) : ['1', '2', '3'];
  ...
  const isCurrentWeekActive = String(currentActiveWeek) === '3';
